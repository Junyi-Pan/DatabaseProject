...
--
DELIMITER $$
CREATE TRIGGER `addReview` AFTER INSERT ON `review` FOR EACH ROW UPDATE movie
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `movieview`  AS SELECT `movie`.`MID` AS `MID`, `movie`.`Name` AS `Name`, `movie`.`Genre` AS `Genre`, `movie`.`Director` AS `Director`, `movie`.`Summary` AS `Summary`, `movie`.`Date` AS `Date`, `movie`.`AID` AS `AID`, `movie`.`Image` AS `Image`, `movie`.`Rating` AS `Rating` FROM `movie` ;
